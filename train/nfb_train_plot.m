...
if strcmp(vol,'group')
    ...
    for i = meas
        for t = 1:numel(tasks)
            pl{1}.(i{:}).(['m' num2str(t)]) = squeeze(mean(res{t}.(i{:})(1:nc,1:end,:), 1));
            pl{1}.(i{:}).(['s' num2str(t)]) = squeeze(std(res{t}.(i{:})(1:nc,1:end,:), 1))/sqrt(nc);
            
            pl{2}.(i{:}).(['m' num2str(t)]) = squeeze(mean(res{t}.(i{:})(nc+1:end,:,:), 1));
        ...
        for d = [0 1]
            ...
            for ip = 2;%1:numel(gr)
                ...
                for t = 1:numel(tasks)                    
                    ...
                    errorbar(0:size(toplot.(['m' num2str(t)]),1)-1,toplot.(['m' num2str(t)])(:,pl_ind{d+1}(end)),toplot.(['s' num2str(t)])(:,pl_ind{d+1}(end)),lsp{t}{ip,pl_ind{d+1}(end)},'Color',col{t}{ip,pl_ind{d+1}(end)},'LineWidth',2,'DisplayName',leg{pl_ind{d+1}(end)});
